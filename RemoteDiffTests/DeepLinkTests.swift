...
final class DeepLinkTests: XCTestCase {
    ...
    func testBasicDeepLink() {
        let url = URL(string: "remotediff://open?host=mac-studio&path=Development/growthq/portal")!
        let link = DeepLink.parse(from: url)

        XCTAssertNotNil(link)
        XCTAssertEqual(link?.host, "mac-studio")
        XCTAssertEqual(link?.repoPath, "Development/growthq/portal")
    ...
    func testUserAtHost() {
        let url = URL(string: "remotediff://open?host=ernesto%40mac-studio&path=Development/growthq/portal")!
        let link = DeepLink.parse(from: url)
